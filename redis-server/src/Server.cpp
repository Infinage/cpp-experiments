...
namespace Redis {
    ...
    std::string Server::handleCommandPing(std::vector<std::string> &args) {
        if (args.size() == 1) {
    ...
    std::string Server::handleCommandEcho(std::vector<std::string> &args) {
        if (args.size() == 2) {
            return Redis::VariantRedisNode(args.back()).serialize();
        } else {
    ...
    std::string Server::handleCommandSet(std::vector<std::string> &args) {
        if (args.size() >= 3) {
    ...
    std::string Server::handleCommandGet(std::vector<std::string> &args) {
        if (args.size() == 2) {
            std::string &key {args[1]};
    ...
    std::string Server::handleCommandExists(std::vector<std::string> &args) {
        long result {0};
    ...
    std::string Server::handleCommandDel(std::vector<std::string> &args) {
        long result {0};
        for (std::size_t i{1}; i < args.size(); i++) {
            std::string &arg {args[i]};
            if (cache.exists(arg)) {
    ...
    std::string Server::handleCommandLAdd(std::vector<std::string> &args, long by) {
        if (args.size() == 2) {
            std::string &key {args[1]};
    ...
    std::string Server::handleCommandTTL(std::vector<std::string> &args) {
       if (args.size() == 2) {
            std::string &key {args[1]};
    ...
    std::string Server::handleCommandLRange(std::vector<std::string> &args) {
        if (args.size() == 4) {
            long left, right;
            std::string &key {args[1]};
    ...
    std::string Server::handleCommandPush(std::vector<std::string> &args, bool pushBack) {
        if (args.size() >= 3) {
            std::string &key {args[1]};
    ...
    std::string Server::handleCommandLLen(std::vector<std::string> &args) {
        if (args.size() == 2) {
            std::string &key {args[1]};
            if (!cache.exists(key) || cache.expired(key)) {
    ...
    std::string Server::handleCommandSave(std::vector<std::string> &args, bool background) {
        if (args.size() != 1) {
            return Redis::PlainRedisNode("ERR wrong number of arguments for command", false).serialize();
        } else if (background) {
            int pid {fork()};
                ...
                // Recv from client and process it
                    ...
                    if (!status) {
                        ...
                        if (request == "-Error receiving data\r\n") {
                            ...
                            std::string serializedResponse {handleRequest(request)};
