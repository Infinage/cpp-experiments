        ...
        [[nodiscard]] std::string extract(const std::string &objectHash) const {
            ...
            while (true) {
                ...
                if (type >= 1 && type <= 4) {
                    ...
                    deltaChain.emplace_back(0, ifs.tellg(), 0);
                    offset -= relOffset;
                } 
            ...
            while (!deltaChain.empty()) {
                ...
                if (type == 0) {
                    ...
                    if (op & 128) {
                        ...
                        if (op &  2) copyOffset |= static_cast<std::size_t>(pop_front(dsv) <<  7);
                        if (op &  4) copyOffset |= static_cast<std::size_t>(pop_front(dsv) << 14);
                        if (op &  8) copyOffset |= static_cast<std::size_t>(pop_front(dsv) << 21);
                        if (op & 16)   copySize |= static_cast<std::size_t>(pop_front(dsv) <<  0);
                        if (op & 32)   copySize |= static_cast<std::size_t>(pop_front(dsv) <<  7);
                        if (op & 64)   copySize |= static_cast<std::size_t>(pop_front(dsv) << 14);

                        // If copy size is not set, set default size
                        if (!copySize) copySize = 0x10000;
                        result = result.substr(copyOffset, copySize);
                    }

                    // Insert / add
                    else {
                        result.append(dsv.begin(), dsv.end());
                    }
                }
                ...
                if (result.size() != size)
                    throw std::runtime_error("Incorrect base object size.");
            }
